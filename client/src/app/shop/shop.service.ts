...
export class ShopService extends ShopServiceBase {
  ...
  initialize() {
    ...
    this.getCachedBrands(() => {
      return this.http.get<ProductBrand[]>(configuration.serviceUrls.productsApiUrl + 'api/products/brands');
    }).subscribe(() => {
    }, error => {
      console.log(error);
    });
    ...
    this.getCachedTypes(() => {
      return this.http.get<ProductType[]>(configuration.serviceUrls.productsApiUrl + 'api/products/types');
    }).subscribe(() => {
    }, error => {
      console.log(error);
    });
    ...
    this.shopParams$.pipe(
      ...
      switchMap(shopParams => this.getCachedProducts(
        () => {
          ...
          return this.http.get<IPagination>(configuration.serviceUrls.productsApiUrl + 'api/products', {observe: 'response', params}).pipe(map(r => r.body));
        },
        shopParams,
        true
      ))
  ...
  getProduct(id: number) {
    return this.getCachedProduct(
      (id: number) => {
        return this.http.get<Product>(`${configuration.serviceUrls.productsApiUrl}api/products/${id}`);
      },
      id
    );
  }
